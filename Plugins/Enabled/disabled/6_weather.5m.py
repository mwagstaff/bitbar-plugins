...
api_key = '56400f43ce318dfa68269341ce7ee828' # get yours at https://developer.forecast.io
units = 'si' # set to si for metric, leave blank for imperial

def auto_loc_lookup():
  try:

    # Changed by Mike to always return London if we're in the UK, because connecting to the VPN ruins everything
    if "GMT" in time.tzname or "BST" in time.tzname:
      return {u'loc': u'51.5074,0.1278', u'city': u'London', u'country': u'GB', u'region': u'England', u'hostname': u'host86-168-149-180.range86-168.btcentralplus.com', u'ip': u'86.168.149.180', u'org': u'AS2856 British Telecommunications PLC', u'postal': u'ME15'}
    else:
...
def render_wx():
  ...
  if 'icon' in weather_data and 'temperature' in weather_data:
    print weather_data['icon'] + ' ' + weather_data['temperature'] + ' ' + weather_data['next_hour'] + ' | color=gray '
  else:
    print 'N/A'
