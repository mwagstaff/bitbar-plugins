    ...
    for row in all_rows:
      ...
      if matchfixture and competition and kickofftime and channels:
        ...
        if today:
          ...
          if competition.text.strip() == 'Premier League' or ( 'Champions League' in competition.text.strip() ) or ( 'Europa League' in competition.text.strip() ) or ( 'England' in matchfixture.text.strip() and 'U21' not in matchfixture.text.strip() ):
            fixturesContent += " color=black"
